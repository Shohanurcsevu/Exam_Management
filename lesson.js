...
function initializeLessonPage() {
        ...
        try {
            ...
            if (result.status === 'success') {
                ...
                if (subjectSelectInModal) {
                    ...
                    result.data.forEach(subject => {
                        subjectSelectInModal.innerHTML += `<option value="${subject.id}">${subject.subject_name}</option>`;
                    });
                }
    ...
    const renderTable = (lessons) => {
        ...
        if (lessons.length === 0) {
            lessonsTableBody.innerHTML = '<tr><td colspan="6" style="text-align:center;">No lessons match your criteria.</td></tr>';
            return;
        }
    ...
    // --- Event Listeners (with added safety checks) ---
